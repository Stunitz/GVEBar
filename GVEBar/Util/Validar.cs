...
namespace GVEBar.Util
{
    class Validar
    {
        public static bool ApenasNumerosInteiros(string texto)
        {
            Regex regex = new Regex("[^0-9]"); //regex que corresponde aos números permitidos
            return regex.IsMatch(texto);
        }

        public static bool ApenasNumerosVirgula(string texto)
        {
            Regex regex = new Regex("[^0-9,]"); //regex que corresponde aos números permitidos
            return regex.IsMatch(texto);
        }
        ...
        public static bool ApenasLetras(string texto)
        {
            Regex regex = new Regex("[^a-zA-ZáÁéÉíÍóÓúÚãÃõÕ]"); //regex que corresponde aos números permitidos
            return regex.IsMatch(texto);
        }
